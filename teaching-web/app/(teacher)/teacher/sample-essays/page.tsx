...
export default async function TeacherSampleEssaysPage() {
  ...
  return (
    <div className="px-4 py-6 md:px-8 md:py-8 max-w-5xl mx-auto space-y-6">
      <header>
        <h1 className="text-2xl md:text-3xl font-bold tracking-tight">Sample Essays Bank</h1>
        <p className="text-sm text-muted-foreground mt-1">
          {essays.length} essay mẫu · Theo task × band · Đầy đủ examiner notes
        </p>
      </header>
      ...
      {essays.length === 0 ? (
        <Card>
          <CardContent className="py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground">Chưa có essay. Chạy seed để nạp bank.</p>
          </CardContent>
        </Card>
      ) : (
                      ...
                      <details className="group">
                        <summary className="cursor-pointer list-none">
                          <div className="flex items-start justify-between gap-2 mb-1.5">
                            <p className="text-sm font-semibold line-clamp-2 flex-1">
                              {e.prompt.slice(0, 100)}{e.prompt.length > 100 ? '...' : ''}
                            </p>
                            <BandPill band={e.band} size="sm" />
                          </div>
                          <p className="text-xs text-muted-foreground">{e.wordCount} từ · {e.topics ?? 'general'}</p>
                        </summary>
                        <div className="mt-3 pt-3 border-t space-y-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm leading-relaxed whitespace-pre-wrap">{e.essay}</p>
                          </div>
                          {e.examinerNotes && (
