  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <div className="page-header animate-fade-up">
        <h1 className="page-title">Sample Essays</h1>
        <p className="page-subtitle">{essays.length} essay band 6-9 · Có examiner notes giải thích chấm điểm</p>
      </div>

      {Array.from(byTask.entries()).map(([task, items]) => (
        <div key={task} className="mb-6 animate-fade-up stagger-1">
              ...
              <details key={e.id} className="card p-4 group">
                <summary className="cursor-pointer list-none">
                  <div className="flex items-start justify-between mb-1">
                    <p className="text-sm font-semibold text-ink line-clamp-2 flex-1 pr-3">{e.prompt.slice(0, 120)}{e.prompt.length > 120 ? '...' : ''}</p>
                    <BandPill band={e.band} />
                  </div>
                  <p className="text-xs text-ink-tertiary">{e.wordCount} từ {e.topics && `· ${e.topics}`}</p>
                </summary>
                <div className="mt-3 pt-3 border-t border-surface-border space-y-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-ink leading-relaxed whitespace-pre-wrap font-serif">{e.essay}</p>
                  </div>
                  {e.examinerNotes && (
      ...
      {essays.length === 0 && (
        <div className="card p-16 text-center">
          ...
          <p className="text-ink-secondary">Chưa có essay mẫu.</p>
        </div>
      )}
    </div>
  )
